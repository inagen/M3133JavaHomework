...
public class ExpressionTest extends BaseTest {
        ...
        private void handmade() {
                ...
                testExpression("((10 / 2) / 3)", "10 / 2 / 3",
                                new Divide(new Divide(new Const(10), new Const(2)), new Const(3)), x -> 10 / 2 / 3);
                testExpression("(10 / (3 / 2))", "10 / (3 / 2)",
                                new Divide(new Const(10), new Divide(new Const(3), new Const(2))), x -> 10);
                testExpression("((x * x) + ((x - 1) / 10))", "x * x + (x - 1) / 10",
                                new Add(new Multiply(new Variable("x"), new Variable("x")), new Divide(
                                                new Subtract(new Variable("x"), new Const(1)), new Const(10))),
                                x -> x * x + (x - 1) / 10);
        ...
        private void generated() {
                ...
                testExpression("(x - (x - 1))", "x - (x - 1)", new Subtract(vx, new Subtract(vx, c1)),
                                x -> x - (x - 1));
                ...
                testExpression("(1 + (1 + (2 + 1)))", "1 + 1 + 2 + 1", new Add(c1, new Add(c1, new Add(c2, c1))),
                                x -> 1 + 1 + 2 + 1);
                testExpression("(x - ((1 * 2) + x))", "x - (1 * 2 + x)",
                                new Subtract(vx, new Add(new Multiply(c1, c2), vx)), x -> x - (1 * 2 + x));
                testExpression("(x * (2 / (x - 1)))", "x * (2 / (x - 1))",
                                new Multiply(vx, new Divide(c2, new Subtract(vx, c1))), x -> x * (2 / (x - 1)));
                testExpression("(x / (1 + (2 + 1)))", "x / (1 + 2 + 1)", new Divide(vx, new Add(c1, new Add(c2, c1))),
                                x -> x / (1 + 2 + 1));
                testExpression("((1 * 2) + (2 + 1))", "1 * 2 + 2 + 1", new Add(new Multiply(c1, c2), new Add(c2, c1)),
                                x -> 1 * 2 + 2 + 1);
                testExpression("((2 + 1) - (2 + 1))", "2 + 1 - (2 + 1)", new Subtract(new Add(c2, c1), new Add(c2, c1)),
                                x -> 2 + 1 - (2 + 1));
                testExpression("((x - 1) * (x / 1))", "(x - 1) * (x / 1)",
                                new Multiply(new Subtract(vx, c1), new Divide(vx, c1)), x -> (x - 1) * (x / 1));
                testExpression("((x - 1) / (1 * 2))", "(x - 1) / (1 * 2)",
                                new Divide(new Subtract(vx, c1), new Multiply(c1, c2)), x -> (x - 1) / (1 * 2));
                testExpression("(((x - 1) - 2) + x)", "x - 1 - 2 + x",
                                new Add(new Subtract(new Subtract(vx, c1), c2), vx), x -> x - 1 - 2 + x);
                testExpression("(((1 * 2) + x) - 1)", "1 * 2 + x - 1",
                                new Subtract(new Add(new Multiply(c1, c2), vx), c1), x -> 1 * 2 + x - 1);
                testExpression("(((2 + 1) / 1) * x)", "(2 + 1) / 1 * x",
                                new Multiply(new Divide(new Add(c2, c1), c1), vx), x -> (2 + 1) / 1 * x);
                testExpression("((2 / (x - 1)) / 2)", "2 / (x - 1) / 2",
                                new Divide(new Divide(c2, new Subtract(vx, c1)), c2), x -> 2 / (x - 1) / 2);
        }

        private void testExpression(final String full, final String mini, final Expression actual,
